...
if(is.null(cds)){
  for(c in comps){
      ...
      file.name <- paste(diff.exp.dir,
                         paste("ResDESeq_",condB,"_vs_",condA,".xlsx",sep=""),
                         sep="/")
...
file.name <- paste(counts.dir,"counts_scaled_DESeq.xlsx",sep="/")
...
if(diff.exp == T){
  if(exists("comps") && !is.null(comps) && !is.null(key)){
    ...
    for (comp in comps){
      ...
      file.name <- paste(diff.exp.dir,
                         paste("ALLResDESeq_",condB,"_vs_",condA,".xlsx",sep=""),
                         sep="/")
        ...
        if(exists("species") & !is.null(species)){
          ...
          if(!is.null(gsa.res$dn)){
            out.file <- file.path(gsa.dir,
                              paste("GeneSet_Dn_",condB,"_vs_",condA,".xlsx",sep="")
                        )
            bic.write.dat(gsa.res$dn,file=out.file)
          }
          if(!is.null(gsa.res$up)){
            out.file <- file.path(gsa.dir,
                              paste("GeneSet_Up_",condB,"_vs_",condA,".xlsx",sep="")
                        )
