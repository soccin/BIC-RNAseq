...
foreach my $sample (keys %samp_libs_run){
	...
	if($species =~ /hg19|human/i){	
		...
		`/common/sge/bin/lx24-amd64/qsub -N $pre\_$uID\_FC_$sample -hold_jid $pre\_$uID\_CAT_$sample -pe alloc 6 -l virtual_free=2G $Bin/qCMD_FC $FUSIONCATCHER/bin/fusioncatcher -d $FUSIONCATCHER/data/ensembl_v77d $inReads -o $output/fusion/fusioncatcher/$sample -p 6`;
