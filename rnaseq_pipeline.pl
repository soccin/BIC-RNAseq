...
if(!$map || !$species || !$strand || !$config || !$scheduler || $help){
	...
	* SPECIES: only hg19 and mouse (mm10; default) and human-mouse hybrid (hybrid) currently supported (REQUIRED)
...
if($species =~ /human|hg19/i){
    ...
    $BOWTIE_INDEX = '/ifs/depot/assemblies/M.musculus/mm9/index/bowtie/1.0./mm9_bowtie';
